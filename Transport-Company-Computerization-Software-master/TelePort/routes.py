...
def signup_customer():
    # emp=Manager.query.delete()   #TODO: To be Removed
    # #emp.name="Changed"
    # db.session.commit()
    form = signUpCustomer()
    if form.validate_on_submit():
...
def help(): #TODO: To be removed
    #x = session.query(Customer).filter_by(customerID=7).first()
    #x = Customer.query.filter_by(customerID=7).first()
    #x.name="Akshay Sankhla"
